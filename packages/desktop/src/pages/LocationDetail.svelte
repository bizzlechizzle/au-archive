<script lang="ts">
  ...
  /**
   * Kanye6: Set an image as the hero image for this location
   * Hero image is displayed prominently at the top of the location detail
   */
...
<div class="h-full overflow-auto">
  ...
  {:else}
    <div class="max-w-6xl mx-auto p-8">
      <!-- Hero Image - Kanye6: Show selected hero image or first image -->
      {#if images.length > 0}
        {@const heroImage = location?.hero_imgsha
          ? images.find(img => img.imgsha === location.hero_imgsha) || images[0]
          : images[0]}
        {@const heroIndex = images.findIndex(img => img.imgsha === heroImage.imgsha)}
        ...
        <div class="mb-6 -mx-8 -mt-8">
          <button
            onclick={() => openLightbox(heroIndex >= 0 ? heroIndex : 0)}
          ...
          {#if images.length > 0}
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
              {#each displayedImages as image, displayIndex}
                ...
                {@const isHero = location?.hero_imgsha === image.imgsha}
                <div class="aspect-square bg-gray-100 rounded overflow-hidden relative group">
                  ...
                  <div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition">
                    ...
                    {:else}
                      <button
                        onclick={(e) => { e.stopPropagation(); setHeroImage(image.imgsha); }}
